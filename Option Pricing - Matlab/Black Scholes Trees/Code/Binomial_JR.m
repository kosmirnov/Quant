function [Price_JR_Tree, Price_JR, Price_BSM] = Binomial_JR(S,X,r,q,sig,T,n,flg,ind)
...
if ind == 0 %European type
    if flg == -1 %Put
        ...
        Price_BSM = BSMP(S,X,r,q,sig,0,T);
    else %Call
        DerivatePrices(1:n+1,n+1) = max(Stock(1:n+1,n+1)-X,0);
        Price_BSM = BSMC(S,X,r,q,sig,0,T);
    end;
